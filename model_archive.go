...
func (a *archive) getArFile(db *sql.DB) error {
	...
	return db.QueryRow("SELECT id, archive_id, date, file_name, language, extension, size, sha1 FROM archive WHERE archive_id = $1",
		a.ArchiveID).Scan(&a.ID, &a.ArchiveID, &a.Date, &a.FileName, &a.Language, &a.Extension, &a.Size, &a.Sha1)
}
